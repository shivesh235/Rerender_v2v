...
model = MIRNet(num_rrg=3, num_mrb=2, channels=64).to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)

# Load model weights if available
model.load_state_dict(torch.load("mirnet_model.pth"))

# Training loop
for epoch in range(20):
    model.train()
    for low, high in train_loader:
        low, high = low.to(device), high.to(device)  # Move data to GPU
        optimizer.zero_grad()
        output = model(low)
        loss = charbonnier_loss(high, output)
        loss.backward()
        optimizer.step()
    print(f"Epoch: {epoch+1}, Loss: {loss.item():.4f}")
    torch.save(model.state_dict(), f"mirnet_model_{epoch}.pth")

# Save final model
torch.save(model.state_dict(), "mirnet_model.pth")
