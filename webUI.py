...
def super_resolve_image(input_image):
    """
    Perform super-resolution on the input image using NinaSR.
    ...
    # Load pretrained NinaSR model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = ninasr_b0(scale=2, pretrained=True).to(device)
    model.eval()

    # Convert input image to tensor and process with the model
    ...
    # Convert the PIL image back to a numpy array for Gradio output
    return np.array(sr_image)

# Low light function
from low_light.infer import MIRNet

def preprocess_image(input_image):
    """
    Preprocess the input image for the MIRNet model.

    Args:
        input_image (PIL.Image): Input low-light image.

    Returns:
        torch.Tensor: Preprocessed image tensor.
    """
    # Convert the image to a tensor and normalize it to [0, 1]
    image_tensor = ToTensor()(input_image).unsqueeze(0)  # Add batch dimension
    return image_tensor

def postprocess_image(output_tensor):
    """
    Postprocess the output tensor from the MIRNet model.

    Args:
        output_tensor (torch.Tensor): Raw output tensor from the model.

    Returns:
        PIL.Image: Postprocessed image as a PIL.Image.
    """
    # Remove batch dimension and rescale from [0, 1] to [0, 255]
    output_image = output_tensor.squeeze(0).clamp(0, 1).cpu()
    output_image = ToPILImage()(output_image)
    return output_image

def infer_lowlight_image(input_image):
    """
    Enhance a low-light image using the trained MIRNet model.

    Args:
        input_image (PIL.Image): Low-light input image.

    Returns:
        PIL.Image: Enhanced image as a PIL.Image.
    """
    # Initialize model, optimizer, and loss
    mirnet = MIRNet(num_rrg=3, num_mrb=2, channels=64)
    optimizer = torch.optim.Adam(mirnet.parameters(), lr=1e-4)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    mirnet.load_state_dict(torch.load("low_light/mirnet_model_50.pth"))
    mirnet.eval()  # Set model to evaluation mode
    ...
    with torch.no_grad():
        enhanced_image = mirnet(preprocessed_image)
    
    # Postprocess the output image
    output_image = postprocess_image(enhanced_image)
...
with block:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            superres_button = gr.Button(value='Super-Resolve First Frame')
            lowlight_button = gr.Button(value='Enhance Low-Light Image')  # New button
        ...
        #     result_image = gr.Image(label='Output first frame', type='numpy', interactive=False)
        #     superres_button = gr.Button(value='Super-Resolve First Frame')
        #     # superres_image = gr.Image(label="Super-Resolved Image", type="numpy")
        #     result_keyframe = gr.Video(label='Output key frame video', format='mp4', interactive=False)
        #     result_video = gr.Video(label='Output full video', format='mp4', interactive=False)

        # Handle input changes for video path
        def input_uploaded(path):
            frame_count = get_frame_count(path)
            if frame_count <= 2:
                raise gr.Error('The input video is too short! Please input another video.')
            ...
            return gr.Slider.update(value=default_interval, maximum=max_keyframe), gr.Slider.update(value=max_keyframe, maximum=max_keyframe)
        ...
        # Set up input path event handlers
        ...
        # Define the logic for the super-resolve button click
        def super_resolve_button_clicked(input_image):
            # When super-resolve button is clicked, apply the super-resolve function
            if input_image is not None:
                return super_resolve_image(input_image)
            else:
                return None

        # Click handler for super-resolve button
        superres_button.click(
            fn=super_resolve_button_clicked, 
            inputs=[result_image], 
            outputs=[result_image]
        )

        # Define the logic for the low-light enhancement button click
        def lowlight_button_clicked(input_image):
            # When low-light enhancement button is clicked, apply the low-light enhancement function
            if input_image is not None:
                return infer_lowlight_image(input_image)  # Use the low-light enhancement function
            else:
                return None

        # Click handler for low-light enhancement button
        lowlight_button.click(
            fn=lowlight_button_clicked, 
            inputs=[result_image], 
            outputs=[result_image]
        )

        # Define the process for generating results (normal video processing)
        ips_process3 = [*ips, max_process, use_poisson]
        run_button.click(fn=process, inputs=ips_process3, outputs=[result_image, result_keyframe, result_video])
